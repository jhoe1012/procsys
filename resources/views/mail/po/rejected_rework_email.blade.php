...
<body>
    <div class="container">
        <h2>Action Required: Purchase Order {{ $po_header->po_number }} is {{ $po_header->status }} </h2>
        <p>Dear {{ $requestor }},</p>
        <p>I am writing to inform you that the Purchase Order {{ $po_header->pr_number }}, submitted on
            {{ date('Y-m-d', strtotime($po_header->doc_date)) }}, has been reviewed but cannot be approved in its
            current form. Below are the
            reasons for the rejection and the necessary actions for rework:</p>
        <p><strong>Rejection Reason:</strong> {{ $po_header->workflows[count($po_header->workflows) - 1]->message }}</p>

        <p>Please revise the Purchase Order based on the feedback provided and resubmit it for approval. If you have any
            questions or need further clarification on the issues mentioned, do not hesitate to reach out to me
            directly.</p>
        <div class="details">
