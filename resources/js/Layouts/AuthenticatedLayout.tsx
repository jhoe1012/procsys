...
export default function Authenticated({
  ...
  header,
  children,
}: PropsWithChildren<{ user: AuthUser; header?: ReactNode; menus: any }>) {
    ...
    <div className="min-h-screen bg-gray-100">
      <nav className="bg-[#F8C110] border-b border-gray-100">
            ...
            <div className="flex ">
              ...
              <div className="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                ...
                {menus.map((menu: any, index: number) => (
                  <div
                    key={index}
                    className=" inline-flex items-center px-0 pt-1 border-b-2 text-sm font-medium leading-5 transition duration-150 ease-in-out focus:outline-none">
                    {menu.permissions && (
                      <Dropdown>
                        <Dropdown.Trigger>
        ...
        <div className={(showingNavigationDropdown ? 'block' : 'hidden') + ' sm:hidden'}>
          <div className="pt-2 pb-3 space-y-1">
            ...
            {/* <ResponsiveNavLink href={route('pr.create')} active={route().current('pr.create')}>
      ...
      {header && (
        <header className="bg-white shadow">
          <div className="max-w-7xl mx-auto py-3 px-4 sm:px-4 lg:px-4">{header}</div>
        </header>
      )}
